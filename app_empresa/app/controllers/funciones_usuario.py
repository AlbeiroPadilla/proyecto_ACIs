...
def recibeInsertRegisterUser(usuario, email, password):
    respuestaValidar = validarDataRegisterLogin(usuario, email, password)
        ...
        with conexion_MySQLdb.cursor(dictionary=True) as cursor:
            ...
            if account:
                if check_password_hash(account['password'], pass_actual):
                    # Verificar si new_password y repetir_password están vacías
                    if not new_password or not repetir_password:
                        ...
                        if new_password != repetir_password:
                            ...
                            try:
                                nueva_password = generate_password_hash(new_password, method='scrypt')
                                with connectionBD() as conexion_MySQLdb:
                                    with conexion_MySQLdb.cursor(dictionary=True) as cursor:
                                        querySQL = """
                                            UPDATE usuarios
...
def updatePerfilSinPass(id_usuario, usuario):
    try:
        ...
        print(f"Ocurrió u error en la función ipdatePerfilSinPass: {e}")
        return []
